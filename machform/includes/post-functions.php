 ...
 ********************************************************************************/
	function mf_process_form($dbh,$input){
		...
		$query = "select form_password,form_language,form_review,form_page_total from `".MF_TABLE_PREFIX."forms` where form_id=?";
		...
		if(($is_inserted && !empty($esl_enable) && !empty($form_email) && empty($form_review) && ($form_page_total == 1) && empty($edit_id)) || 
		   ($is_inserted && !empty($esl_enable) && !empty($form_email) && $is_committed && empty($edit_id))
		){
		...
		if(($is_inserted && !empty($esr_enable) && !empty($esr_email_address) && empty($form_review) && ($form_page_total == 1) && empty($edit_id)) || 
		   ($is_inserted && !empty($esr_enable) && !empty($esr_email_address) && $is_committed && empty($edit_id))
		){
			//get parameters for the email
	...
	function mf_commit_form_review($dbh,$form_id,$record_id,$options=array()){
		...
		//unset the captcha_passed session, if any
		$_SESSION['captcha_passed'][$form_id] = false;
		unset($_SESSION['captcha_passed'][$form_id]);
						...
						if($payment_price_type == 'fixed'){ //this is fixed amount payment
							...
							foreach ($price_field_columns as $column_name){
								...
								if(!empty($row[$column_name]) && !in_array($column_name,$processed_column_name)){
									...
									if(!empty($paypal_params['amount_'.$i]) && ($paypal_params['amount_'.$i] !== '0.00')){
										$payment_has_value = true;
									}
